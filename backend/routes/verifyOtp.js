...
router.post("/", validateOtp, async (req, res, next) => {
  ...
  try {
    ...
    // Check OTP
    const [user] = await connection.query("SELECT id, otp FROM registrations WHERE email = ? AND otp = ? AND is_verified = FALSE", [email, otp]);
    ...
    // Mark as verified
    await connection.query("UPDATE registrations SET is_verified = TRUE, otp = NULL WHERE email = ?", [email]);
