...
const validateRegistration = [
  ...
  body('cnic').matches(/^\d{13}$/).withMessage('CNIC must be 13 digits'),
  body('address').trim().notEmpty().withMessage('Address is required'),
  body('city').trim().notEmpty().withMessage('City is required'),
  body('contact').matches(/^\d{11}$/).withMessage('Contact must be 11 digits'),
  body('contact_ii').optional().matches(/^\d{11}$/).withMessage('Secondary contact must be 11 digits'),
